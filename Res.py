...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        layers = []
        layers.append(block(self.inplanes, planes, stride, downsample))  #这里的维度已经变成了 planes*4
        self.inplanes = planes * block.expansion
        for _ in range(1, blocks):
            layers.append(block(self.inplanes, planes))   #所以按照bottleneck类来说的话，每次进入该模块都有一个先降低通道数在变回通道数的过程
