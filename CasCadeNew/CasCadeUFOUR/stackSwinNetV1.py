...
class ResBlock(nn.Module):
    """残差连接块，该残差连接块其实就是resnet的残差块，不过该残差块是设计的3个卷积、归一、激活的块。"""
...
class Hourglass(nn.Module):
    ...
    def __init__(self, n, f, increase=0, up_mode='bilinear',
        ...
        if self.n > 1:
            ...
            # 采用递归的写法，直到变为1就采用残差块。也就是说其中会嵌套3次up1+pool1 + low1 + net + low3,其中net都是一次沙漏块的循环，也就是说其实是逐步的维度变小
        else:
        ...
        for i in range(self.nstack):
            ...
            if i < self.nstack - 1:
                x = x + self.merge_features[i](feature) + \
                    self.merge_heatmaps[i](heatmaps)     # 将三者的维度换到同一个维度，也就是之前的pre的结果的维度
