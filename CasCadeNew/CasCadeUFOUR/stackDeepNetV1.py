...
"""
stackDeepNetV1:主要改动如下：
1、将模型的深度加深，初始单个Hourglass只有13个残差卷积块，同时有四个残差卷积块是用于Encoder和Decoder之间连接用的。，现今在Encoder和Decoder之间各加一层
2、将单个的Hourglass的pool层的变化做更改，即Hourglass的第一个pool层不操作，后续的递归块才应用，即第一次保持尺寸不变，后面才开始尺度变化。
变化后，单个Hourglass一共有21个残差卷积块，有四个残差卷积块用于Encoder和Decoder之间的连接
"""
...
class Hourglass(nn.Module):
    """沙漏网络的设计，注意一点这里的沙漏网络和传统的沙漏网络的区别在何处？上采样方式采用最近邻, 每个沙漏网络原始的设计是只有3个"""
    def __init__(self, n, f, increase=0, up_mode='bilinear', add_coord=False, first_one=False, x_dim=64, y_dim=64):   # nearest'
    ...
    def forward(self, x, heatmap=None):
        ...
        low12 = self.low12(low1)
        low2 = self.low2(low12)
        low3 = self.low3(low2)
        low4 = self.low4(low3)
        if self.n != 4:
            up2 = self.up2(low4)  # 输出的维度和输入的维度明显是一样的
        else:
            up2 = low4
        return up1 + up2
...
class StackedHGNetV1(nn.Module):
    ...
    """应用更改后的沙漏网络进行训练，注意此时pre阶段的块是少了一层的，主要的思考是想要减少pre阶段特征的丢失"""
