...
import dataset
from CasCadeUFOUR.stackNet1 import StackedHGNetV2
...
def train(Dataset, Network):
    # dataset
    cfg = Dataset.Config(datapath='/userHome/zy/Hms/ACCoNet/dataset/train_dataset/EORSSD/train', savepath='/data/Hms/model_newcas/CasCadeUFour-12-10-2',mode='train', batch=2
                         , lr=1e-3, momen=0.9,
    ...
    for epoch in range(cfg.epoch):
        if epoch < 60:
            ...
            if epoch < 10:
                ...
                adjust_lr(optimizer, 1e-4, epoch - 10, decay_rate=10, decay_epoch=20)
            else:
                optimizer.param_groups[0]['lr'] = cfg.lr * 3
            # optimizer.param_groups[0]['lr'] = cfg.lr * 10  # epoch +
        else:
            if epoch < 120:
                optimizer.param_groups[0]['lr'] = cfg.lr
            else:
                adjust_lr(optimizer, cfg.lr, epoch - 70, decay_rate=0.5, decay_epoch=50)
        ...
        if epoch >= 69:
            ...
            if (epoch >= 25 and best_epoch == epoch + 1) or epoch == 41 or epoch == 199:
                ...
                if epoch == 199:
                    jsonnum['lr'] = lrnum
                    jsonnum['loss'] = lossnum.tolist()
                    jsonnum['mse'] = MAEnum.tolist()
                    with open('/data/Hms/model_newcas/CasCadeUFour-12-10-2/data.json', 'w') as f:
                        json.dump(jsonnum, f, cls=MyEncoder,indent=4, ensure_ascii=False)
if __name__ == '__main__':
    os.environ["CUDA_VISIBLE_DEVICES"] = '5'
    train(dataset, StackedHGNetV2)